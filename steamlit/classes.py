...
# API_KEY="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"
...
class TopicExplainer(ClientBase):
    ...
    def get_response(self, *args) -> str:
        """
        pass only 1 string argument to use normally for response, pass in two
        arguments to make it behave as initial response for subject and topic
        Returns the AI's initial response explaining the subject and topic.
        """
        if len(args)==2:
            subject, topic = args
            return super().get_response(f"Subject: {subject}\nTopic: {topic}")
        else:
            return super().get_response(args[0])
